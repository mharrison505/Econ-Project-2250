...
```{r}
#length(New2021NFL$Win6)
#length(New2021NFL$`PA/G`)
#length(New2021NFL$Tm.5)

names(New2021NFL)[names(New2021NFL) == "PA/G"]  <- "PA"      # Points allowed per game
names(New2021NFL)[names(New2021NFL) == "Tm.5"]  <- "Tm_PA"   # Team names associated with PA

#Use the final wins column
New2021NFL$Wins <- New2021NFL$Win6

#Base plot
plot(New2021NFL$Wins,
     New2021NFL$PA,
     xlab = "Wins",
     ylab = "Points Allowed Per Game",
     main = "Wins vs Points Allowed (2021)")

text(New2021NFL$Wins,
     New2021NFL$PA,
     labels = New2021NFL$Tm_PA,
     pos = 3,
     cex = 0.5)

#Regression model
model <- lm(PA ~ Wins, data = New2021NFL)
abline(model, col = "green", lwd = 2)
